...
import { Textarea } from "@/components/ui/textarea"; // Still needed for other Textareas if any, but not for variant.value
...
const AllVariantsPage = () => {
  ...
  const fetchAllVariants = useCallback(async () => {
    ...
    try {
      ...
      setAllVariants([]);
      setError(err.message || "Failed to fetch variants. Please try again.");
  ...
  const handleCreateVariant = async (e: React.FormEvent) => {
    ...
    for (const detail of createForm.value) {
      if ((detail.key.trim() && !detail.value.trim()) || (!detail.key.trim() && detail.value.trim())) {
        toast.error("All Value fields must have both a key and a value, or be completely empty.");
        setIsCreating(false);
        return;
      }
    }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {loading ? (
          <div className="flex justify-center items-center h-64">
            ...
            <p className="ml-3 text-muted-foreground">Loading variants...</p>
          </div>
                    ...
                    <TableRow key={variant.id}>
                      ...
                      <TableCell className="text-right">
                        <div className="flex justify-end space-x-2">
                          ...
                          >
                            {isDeleting && variantToDeleteId === variant.id ? (
